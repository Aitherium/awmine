...
Rules:
  * **MOAT001** no monorepo import (`lib.`, `services.`, `from AitherOS`). awmine
...
archive) — never 0 for "I could not look".
    ...
    python scripts/check_moat_boundary.py [dist/awmine-*.whl dist/*.tar.gz ...]
...
def _entries(path: Path) -> List[Tuple[str, Callable[[], bytes]]]:
    ...
    try:
        if path.suffix == ".whl" or path.suffix == ".zip":
            ...
            return [(n, (lambda n=n: zf.read(n))) for n in zf.namelist() if not n.endswith("/")]
...
def inspect(path: Path) -> List[str]:
    ...
    for name, read in entries:
        ...
        if _MONOREPO_IMPORT.search(blob):
            findings.append(
                f"MOAT001 {path.name}:{name} imports the monorepo "
        ...
        for pattern, label in _INTERNAL:
            for hit in set(pattern.findall(blob)):
                findings.append(
                    f"MOAT002 {path.name}:{name} leaks an {label}: {hit.decode('utf-8', 'replace')}"
                )

    for keystone in _KEYSTONES:
        if keystone not in seen:
            findings.append(
                f"MOAT003 {path.name} is MISSING {keystone} — the artifact "
    ...
    if argv:
        ...
        if missing:
            raise CouldNotJudgeError(
                "named artifact(s) not found: " + ", ".join(str(m) for m in missing)
            )
        return paths
    found = [p for p in (_newest("*.whl"), _newest("*.tar.gz")) if p]
    if not found:
        raise CouldNotJudgeError(
            "no wheel or sdist in dist/ — nothing was built, so "
...
def self_test() -> int:
    ...
    def wheel(members: dict) -> Path:
        p = Path(tempfile.mkdtemp()) / "awmine-0.0.0-py3-none-any.whl"
    ...
    check(
        "MOAT001 catches a monorepo import",
        any(
            f.startswith("MOAT001")
            for f in inspect(wheel({**clean, "awmine/x.py": "from lib.core import X\n"}))
        ),
        True,
    )

    check(
        "MOAT002 catches a debt id",
        any(
            "debt-ledger row id" in f
            for f in inspect(wheel({**clean, "awmine/x.py": "# see D-0000\n"}))
        ),
        True,
    )

    check(
        "MOAT002 catches a checker rule id",
        any(
            "checker rule id" in f
            for f in inspect(wheel({**clean, "awmine/x.py": "# MOAT001 says so\n"}))
        ),
        True,
    )
    ...
    check(
        "MOAT003 refuses an artifact missing a keystone",
        any(f.startswith("MOAT003") for f in inspect(wheel({"awmine/cli.py": "x=1\n"}))),
        True,
    )

    # The package's own imports should not be flagged.
    check(
        "does NOT flag the package's own imports",
        inspect(wheel({**clean, "awmine/y.py": "from awmine.client import call\n"})) == [],
        True,
    )
    ...
    try:
        broken = Path(tempfile.mkdtemp()) / "awmine-0.0.0.tar.gz"
...
def main(argv: List[str]) -> int:
    ...
    if findings:
        print(f"\nmoat guard: {len(findings)} violation(s) — NOT publishing", file=sys.stderr)
        for f in findings:
    ...
    print(
        f"moat guard: clean — {names} carry no monorepo import, no internal "
